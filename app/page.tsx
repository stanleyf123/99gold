  ...
  return (
    <main>
      ...
      <section className="marketHub" id="top"><div className="hubLead"><div><p className="eyebrow">GOLD MARKET DASHBOARD</p><h1>{copy.dashboard}</h1><p>{copy.dashboardText}</p></div><div className="hubPrice"><span>{quotes[0]?.label}・{quotes[0]?.code}</span><strong>{quotes[0]?.price ?? "—"}</strong><em className={quotes[0]?.up ? "up" : "down"}>{quotes[0]?.up ? "▲" : "▼"} {quotes[0]?.change}</em></div></div><div className="marketTabs" role="tablist" aria-label="黃金資訊分類"><button role="tab" aria-selected={activeTab === "quotes"} className={activeTab === "quotes" ? "active" : ""} onClick={() => setActiveTab("quotes")}>{copy.quotes}</button><button role="tab" aria-selected={activeTab === "history"} className={activeTab === "history" ? "active" : ""} onClick={() => setActiveTab("history")}>{copy.history}</button><button role="tab" aria-selected={activeTab === "tools"} className={activeTab === "tools" ? "active" : ""} onClick={() => setActiveTab("tools")}>{copy.tools}</button><button role="tab" aria-selected={activeTab === "news"} className={activeTab === "news" ? "active" : ""} onClick={() => setActiveTab("news")}>{copy.news}</button></div>
        ...
        {activeTab === "news" && <div className="hubPanel newsPanel" role="tabpanel"><div className="panelHeading"><div><p className="eyebrow">TODAY&apos;S MARKET FOCUS</p><h2>{copy.news}</h2></div><p>{newsUpdated}</p></div><p className="panelIntro">每日整理黃金、美元、利率與國際財經焦點；點擊可查看摘要及原始報導。</p><div className="newsFilters" role="tablist" aria-label="新聞分類">{newsCategories.map((category) => <button key={category} className={newsCategory === category ? "active" : ""} onClick={() => setNewsCategory(category)}>{category}</button>)}</div><div className="newsGrid">{filteredNews.slice(0, 10).map((item, i) => { const category = classifyNews(item.title); const key = String(item.id ?? item.url); const cover = item.image || categoryCover[category]; return <article key={key}><div className="newsVisual hasImage"><img src={cover} alt="" loading="lazy" referrerPolicy="no-referrer" onError={(event) => { if (event.currentTarget.src !== categoryCover[category]) event.currentTarget.src = categoryCover[category]; else event.currentTarget.style.display = "none"; }}/></div><p><b>{category}</b><time>{item.date}</time></p><h3>{item.title}</h3><a href={item.id ? `/news/${item.id}` : item.url} target={item.id ? undefined : "_blank"} rel={item.id ? undefined : "noreferrer"}>{copy.read}　→</a></article>; })}</div>{filteredNews.length === 0 && <p className="newsEmpty">這個分類目前沒有新消息，請切換「全部」查看其他市場焦點。</p>}</div>}
