...
type NewsItem = { id?: number; title: string; date: string; url: string; image?: string };
...
export default function Home() {
  ...
  const filteredNews = news.filter((item) => Boolean(item.id) && !hiddenNews.includes(String(item.id)) && (newsCategory === "全部" || classifyNews(item.title) === newsCategory));
  ...
  return (
    <main>
      ...
      <section className="marketHub" id="top"><div className="hubLead"><div><p className="eyebrow">GOLD MARKET DASHBOARD</p><h1>{copy.dashboard}</h1><p>{copy.dashboardText}</p></div><div className="hubPrice"><span>{quotes[0]?.label}・{quotes[0]?.code}</span><strong>{quotes[0]?.price ?? "—"}</strong><em className={quotes[0]?.up ? "up" : "down"}>{quotes[0]?.up ? "▲" : "▼"} {quotes[0]?.change}</em></div></div><div className="marketTabs" role="tablist" aria-label="黃金資訊分類"><button role="tab" aria-selected={activeTab === "quotes"} className={activeTab === "quotes" ? "active" : ""} onClick={() => setActiveTab("quotes")}>{copy.quotes}</button><button role="tab" aria-selected={activeTab === "history"} className={activeTab === "history" ? "active" : ""} onClick={() => setActiveTab("history")}>{copy.history}</button><button role="tab" aria-selected={activeTab === "tools"} className={activeTab === "tools" ? "active" : ""} onClick={() => setActiveTab("tools")}>{copy.tools}</button><button role="tab" aria-selected={activeTab === "news"} className={activeTab === "news" ? "active" : ""} onClick={() => setActiveTab("news")}>{copy.news}</button></div>
        ...
        {activeTab === "news" && <div className="hubPanel newsPanel" role="tabpanel"><div className="panelHeading"><div><p className="eyebrow">TODAY&apos;S MARKET FOCUS</p><h2>{copy.news}</h2></div><p>{newsUpdated}</p></div><p className="panelIntro">僅刊登已驗證來源、語系與原始封面圖片的站內新聞內容。</p><div className="newsFilters" role="tablist" aria-label="新聞分類">{newsCategories.map((category) => <button key={category} className={newsCategory === category ? "active" : ""} onClick={() => setNewsCategory(category)}>{category}</button>)}</div><div className="newsGrid">{filteredNews.slice(0, 10).map((item, i) => { const category = classifyNews(item.title); const key = String(item.id); return <article key={key}><div className="newsVisual hasImage"><img src={item.image} alt="" loading="lazy" referrerPolicy="no-referrer" onError={() => setHiddenNews((current) => current.includes(key) ? current : [...current, key])}/><span>{String(i + 1).padStart(2, "0")}</span></div><p><b>{category}</b><time>{item.date}</time></p><h3>{item.title}</h3><a href={`/news/${item.id}`}>{copy.read}　→</a></article>; })}</div>{filteredNews.length === 0 && <p className="panelIntro">目前沒有通過來源、語系與原始圖片驗證的新聞，稍後會自動重新檢查。</p>}</div>}
