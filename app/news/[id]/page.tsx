...
async function getArticle(id:string){return getRawDb().prepare("SELECT * FROM news_articles WHERE id = ?").bind(id).first<Article>();}
export async function generateMetadata({params}:{params:Promise<{id:string}>}):Promise<Metadata>{
 const id=(await params).id;
 const article=editorials.find(a=>a.id===id);
 if(article)return {title:article.title,description:article.description,alternates:{canonical:"https://99gold.net/news/"+id,languages:Object.fromEntries(editorials.filter(a=>a.group===article.group).map(a=>[a.locale==="zh"?"zh-Hant":a.locale,"https://99gold.net/news/"+a.id]))},openGraph:{type:"article",title:article.title,description:article.description,images:[{url:"https://99gold.net"+article.image,width:1536,height:1024,alt:article.imageAlt}]},twitter:{title:article.title,description:article.description,images:["https://99gold.net"+article.image]}};
 ...
 return {title:row.title,description:row.body.slice(0,150),alternates:{canonical:"https://99gold.net/news/"+row.id,languages:Object.fromEntries(related.map(r=>[r.locale==="zh"?"zh-Hant":r.locale,"https://99gold.net/news/"+r.id]))},openGraph:{title:row.title,description:row.body.slice(0,150),images:[]},twitter:{title:row.title,description:row.body.slice(0,150),images:[]}};
}
