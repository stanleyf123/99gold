...
const ADMIN_EMAIL = "stanleys1225@gmail.com";
...
export async function GET() {
  ...
  try {
    const [candidates, sources, runs] = await Promise.all([
      env.DB.prepare(`SELECT id, title, summary, canonical_url, source_name, category,
      ...
      env.DB.prepare(`SELECT source_id, last_attempt_at, last_success_at, last_error, consecutive_errors
        FROM news_source_state ORDER BY source_id`).all(),
      env.DB.prepare(`SELECT id, trigger, scheduled_for, finished_at, status, sources_checked,
...
export async function POST() {
  ...
  if ("error" in auth) return auth.error;
  try {
    const result = await runNewsPipeline(env.DB, new Date(), "manual");
...
export async function PATCH(request: Request) {
  ...
  if (body.action === "reject") {
    const result = await env.DB.prepare(`UPDATE news_candidates SET
