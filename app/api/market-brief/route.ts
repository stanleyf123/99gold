...
export async function GET() {
  try {
    const response = await fetch(`${feedUrl}&_=${Date.now()}`, {
      cache: "no-store",
      headers: { "User-Agent": "GoldenTideMarketBrief/1.0" },
    });
    if (!response.ok) throw new Error("Market brief unavailable");
    ...
    return NextResponse.json(
      {
        items,
        updatedAt: `新聞更新：${new Intl.DateTimeFormat("zh-TW", { hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: false, timeZone: "Asia/Taipei" }).format(new Date())}`,
      },
      { headers: { "Cache-Control": "no-store, max-age=0", "CDN-Cache-Control": "no-store" } },
    );
  } catch {
    return NextResponse.json({ items: [], updatedAt: "新聞來源暫時無法連線，將自動重試" }, { status: 503, headers: { "Cache-Control": "no-store, max-age=0" } });
  }
}
