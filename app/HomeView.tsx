...
import { formatTwdAmount, jewelrySellFromBuy, parseQuotedNumber } from "../lib/section-quotes";
...
function normalizeMetal(metal: {
  ...
  const numberOrNaN = (value: number | null | undefined) => Number.isFinite(value) ? Number(value) : Number.NaN;
  return {
    ...metal,
...
export default function HomeView({ initialQuotes = null }: { initialQuotes?: HomeQuoteSnapshot | null }) {
  ...
  const [marketStatus, setMarketStatus] = useState<MarketStatus>(
    initialQuotes?.items?.length ? (initialQuotes.marketStatus ?? "delayed") : "checking",
  );
  const [quoteCheckFailed, setQuoteCheckFailed] = useState(false);
  const [quoteSource, setQuoteSource] = useState(
    initialQuotes?.quoteSource ?? initialQuotes?.source ?? "Yahoo Finance GC 黃金期貨與公開匯率資料",
  );
  ...
  const quotesPending = quotes.length === 0 && !quoteCheckFailed && (marketStatus === "checking" || !quoteAttemptedAt);
  const quotesUnavailable = quotes.length === 0 && !quotesPending;
    ...
    <main lang={locale === "zh" ? "zh-Hant" : locale}>
      ...
      <section className="brandHero" aria-label={`${siteSettings.fullName}｜${siteSettings.tagline}`}>
        <div className="heroPhoto" aria-hidden="true">
          <Image className="brandHeroPhoto" src="/og.jpg" alt="" fill priority unoptimized sizes="(max-width: 900px) 100vw, 62vw" />
        </div>
      ...
      <section className={`marketRadar${quotesUnavailable || quotesPending ? " radarEmpty" : ""}`}>
        <div>
          <p className="eyebrow">MARKET RADAR</p>
          <h2>{t("今日市場快速判讀", "Today’s market snapshot", "本日の市場早わかり")}</h2>
        </div>
        {quotesPending ? (
          <p className="radarEmptyNote">{t("正在取得即時報價…", "Loading live quotes…", "即時相場を取得中…")}</p>
        ) : quotesUnavailable ? (
      ...
      <section className="marketHub" id="top"><div className="hubLead"><div><p className="eyebrow">GOLD MARKET DASHBOARD</p><h1>{copy.dashboard}</h1><p>{copy.dashboardText}</p></div>{quotes[0] ? <div className="hubPrice"><span>{`${quotes[0].label}・${quotes[0].code}`}</span><strong>{quotes[0].price}</strong><em className={quotes[0].up === null ? "neutral" : quotes[0].up ? "up" : "down"}>{`${quotes[0].up === null ? "•" : quotes[0].up ? "▲" : "▼"} ${quotes[0].change}`}</em></div> : <p className="hubEmptyHint">{quotesPending ? t("正在取得行情", "Loading quotes", "相場を取得中") : t("行情來源暫不可用", "Quote source unavailable", "相場ソースは現在利用できません")}</p>}</div><div className="marketTabs" role="tablist" aria-label="黃金資訊分類"><button role="tab" aria-selected={activeTab === "quotes"} className={activeTab === "quotes" ? "active" : ""} onClick={() => selectDashboardTab("quotes")}>{copy.quotes}</button><button role="tab" aria-selected={activeTab === "history"} className={activeTab === "history" ? "active" : ""} onClick={() => selectDashboardTab("history")}>{copy.history}</button><button role="tab" aria-selected={activeTab === "tools"} className={activeTab === "tools" ? "active" : ""} onClick={() => selectDashboardTab("tools")}>{copy.tools}</button><button role="tab" aria-selected={activeTab === "news"} className={activeTab === "news" ? "active" : ""} onClick={() => selectDashboardTab("news")}>{copy.news}</button></div>
        ...
        {activeTab === "quotes" && (
          <div className="hubPanel proQuotePanel" role="tabpanel" id="quotes">
            ...
            {quotesPending ? (
              <p className="quoteEmptyPanel">{t("正在取得即時報價…", "Loading live quotes…", "即時相場を取得中…")}</p>
            ) : quotesUnavailable ? (
            ...
            <div className="conversionCards">
              {quotes.slice(1).flatMap((quote) => {
                const card = (
                  <article key={quote.id ?? quote.label}>
                ...
                const buy = (quote.id === "taiwan-qian" || quote.label.includes("台灣理論買進"))
                  ? parseQuotedNumber(quote.price)
                  : null;
                if (buy === null) return [card];
                return [card, (
                  <article key="taiwan-sell-estimate">
                    <div><span>{t("台灣理論賣出估計", "Taiwan estimated sell", "台湾の推定売値")}</span><small>{t("理論買進 × 估計溢價 4%", "Theoretical buy × 4% estimated premium", "理論買×推定プレミアム4%")}</small></div>
                    <strong>{formatTwdAmount(jewelrySellFromBuy(buy))}</strong>
                    <div className="conversionFoot"><span>{quote.unit}</span><b>{t("估計溢價 4% · 非店家牌價", "Est. 4% premium · not a shop price", "推定4%・店頭価格ではありません")}</b></div>
                  </article>
                )];
              })}
