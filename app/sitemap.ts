...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try { archived=(await env.DB.prepare("SELECT id FROM news_articles ORDER BY published_at DESC LIMIT 500").all<{id:string}>()).results; } catch { /* Authored articles remain available without the archive database. */ }
