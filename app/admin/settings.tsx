...
export default function AdminSettings({ userName, tokenAuth = false }: { userName: string; tokenAuth?: boolean }) {
  ...
  useEffect(() => { fetch("/api/site-settings", { cache: "no-store" }).then((r) => r.json() as Promise<Partial<Settings>>).then((data) => { setSettings({ ...fallback, ...data }); setStatus("內容已同步"); }).catch(() => setStatus("暫時無法讀取內容")); }, []);
