...
const data=moduleFrom("../app/news/editorial.ts");
const view=moduleFrom("../app/news/EditorialView.tsx",{"./editorial":data});
test("three complete editions have local art, stable dates and cited sources",()=>{
 assert.equal(data.editorials.length,3);
...
test("old events are excluded instead of receiving a new publication date",()=>{
 assert.equal(data.recentEditorials("zh",Date.parse("2026-09-12T12:00:00Z")).length,1);
...
test("reader API returns original work and local images without a translation request",async()=>{
 ...
 for(const lang of ["zh","en","ja"]){const result=await api.getDailyGoldNews(lang);assert.equal(result.items.length,1);assert.ok(result.items[0].id.endsWith("-"+lang));assert.equal(result.items[0].translated,false);assert.ok(result.items[0].image.startsWith("/"));}
});
